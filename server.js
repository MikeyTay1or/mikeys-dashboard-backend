...
app.get('/api/instagram', async (req, res) => {
  try {
    ...
    const [insightsRes, profileRes, mediaRes] = await Promise.all([
      fetch(`https://graph.facebook.com/v19.0/${igId}/insights?metric=impressions,reach,profile_views,follower_count,website_clicks&period=${period}&access_token=${pageToken}`),
      fetch(`https://graph.facebook.com/v19.0/${igId}?fields=followers_count,media_count,biography&access_token=${pageToken}`),
